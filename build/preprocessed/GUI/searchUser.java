...
public class searchUser extends List implements CommandListener, Runnable {
String usersearch;
    Command cmdRefresh = new Command("Rafraichir", Command.SCREEN, 0);
    Command add = new Command("Ajouter User", Command.SCREEN, 0);
     Command back = new Command("Retoure ", Command.SCREEN, 0);
    User[] people;
    StringBuffer sb;
    ID[] numbers;
    Display disp;
    ...
    public searchUser(String title, int listType, Display d) {
        super("Search", listType);
        usersearch=title;
        disp=d;
    ...
    public void run() {
        try {
            ...
            HttpConnection hc = (HttpConnection) Connector.open("http://localhost/parsing2016/searchuser.php?username="+usersearch);//people.xml est un exemple
